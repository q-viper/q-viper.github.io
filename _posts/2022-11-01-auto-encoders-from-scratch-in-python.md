...
I started to write this blog in 2020 May but since I already written above two blogs, I felt that why bother train another model from scratch. But in 2022 November, I got following email from a reader and I thought that could be helpful to someone. 

![]({{site.url}}/assets/cnn/aemail.png)

I really feel grateful that the blogs I written after failing an interview question `Working mechanism of Convolutional Neural Network` are being helpful. **If I have to write Neural Networks from Scratch again then I would have written it with easier and smaller codes.** I feel that there are many things I written in a horrible way 😜.

## Introduction
Before going into auto-encoders from scratch, let me write something fundamental about it. 
* Auto-encoders are nothing but a sequence of neural networks which basically has same output as the input. 
* Auto-encoders are trained to reach one thing that Deep Learning engineers hate, i.e. over-fitting. 

Why do we call it encoder? Well its because it turns our data into some weird form and then could turn that weird form into somewhat original form. In Cryptography, we use plain text with some key to cipher the data to receive data in non readable format to protect it from unwanted readers. And we use some key to decipher that text to original form. And Auto-encoder also has that ability (or concept only). In Cryptography, we can have two parts, one Encoder and another is Decoder. One part encodes data and another part decodes data from encoded data to give original data. Similarly, Auto-encoder is made up of two logical parts, Encoder and Decoder. Encoder is a small part of this whole network which receives inputs and processes and gives encoded form of input. Then another part Decoder receives that encoded form of input and processes to return original form. The right question here will be where is the encryption key? There is not any. The encryption key in this model is a weights or parameters of a model.

In normal ML training, we do not want a model to be over-fitted or under-fitted. But in Auto-encoder, its better to be over-fitted. Because we want our neural network to remember some information.
...
* What if we have a powerful auto-encoder that was trained on a series of our image. The overall size of image is huge because they were all captured with really high quality camera. We can train an auto-encoder with many parameters and over-fit them to learn every image possible. Now we can have a very small output of encoder and save that into the simple CSV file and delete the images. What is important here is storing the parameters and architecture of model. If we want to view some image, we could simply pass the few bytes of encoded data to decoder and view the original image. Just my thought though!
...
## Coding Part
Since auto-encoders are made up of sequence of neural networks, auto-encoders form scratch simply means neural networks from scratch and here my coding goal will be to use my already created Neural Network module from scratch. Please find the codes in my GitHub repository below:
...
### Getting Data
To get the minst data, we are going to use Keras. I feel it easy.
...
We will create model with 3 layers. One will be input and then there will be second layer and the final output layer. For us the first two layers could be encoder and last one could be decoder. But it is not necessary to divide like that. We will train for only 10 epochs with below hyper-parameters.
...
It took my machine 23min to complete above code and may be this machine is old as well as my code. The accuracy was low but it doesn't matter much in Auto-encoders. Only thing that matters is loss and loss was decreasing slowly.

## Input vs Output
We trained a model but was it strong enough to predict? Lets figure it out.
...
It seems that our model was able to predict something what we can guess but it still is not okay! May be it could do better after training for more epochs but I would like to conclude it here.

## Conclusion
We were able to train a simple Auto Encoder from Scratch and see the results and I hope you found this to be useful.
